...
public class flinkNetworkStatus {
    ...
    public static void main(String[] args) throws Exception {
        ...
        properties.setProperty("group.id", "flink-kafka-network-status");

        // 3. 创建Kafka Source
        FlinkKafkaConsumer<String> consumer = new FlinkKafkaConsumer<>(
                "network_status",                             // 源 topic
...
//        kafkaStream.print();
        // 5. 进行数据清洗
        SingleOutputStreamOperator<NetworkStatus> parsedStream = kafkaStream
                .map(new NetworkStatusMap())
...
//        postgresSource.print();
        ...
        DataStream<NetworkStatus> processed = parsedStream
                .connect(broadcastStream)
                .process(new BroadcastProcessFunction<NetworkStatus, Map<String, String> ,NetworkStatus>() {

                    @Override
                    public void processElement(NetworkStatus networkStatus, BroadcastProcessFunction<NetworkStatus, Map<String, String>, NetworkStatus>.ReadOnlyContext ctx, Collector<NetworkStatus> out) throws Exception {
                        ReadOnlyBroadcastState<String, String> broadcastState =
                                ctx.getBroadcastState(broadcastStateDescriptor);
                        String sidName = broadcastState.get(networkStatus.sidName());
                        if(!sidName.isEmpty()){
                            networkStatus.sidName(sidName);
                        }
                        out.collect(networkStatus);
                    }

                    @Override
                    public void processBroadcastElement(Map<String, String> stringStringMap, BroadcastProcessFunction<NetworkStatus, Map<String, String>, NetworkStatus>.Context ctx, Collector<NetworkStatus> out) throws Exception {
        ...
        // 9. 存入 redis
        processed.addSink(
                new RedisSinkFunction("network_status", new NetworkStatusKeyExtractor()))
                .name("Redis Sink");
    ...
    public static class NetworkStatusKeyExtractor implements Function<NetworkStatus, String>, Serializable {

        private static final long serialVersionUID = 1L;
        @Override
        public String apply(NetworkStatus source) {
